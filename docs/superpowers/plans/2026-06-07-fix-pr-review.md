# Fix PR Review Command Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a `.claude/commands/fix-pr-review.md` slash command that fetches PR Agent review comments, filters by importance threshold, and applies relevant fixes with one commit per fix.

**Architecture:** A single Claude Code custom command file. When invoked as `/fix-pr-review <url-or-number> [threshold]`, Claude reads the file as instructions, fetches the PR with `gh`, parses the PR Agent comment blocks, filters issues by relevance, applies fixes (using suggestions as references, not strict diffs), and commits each fix.

**Tech Stack:** Claude Code custom commands (`.claude/commands/`), `gh` CLI, `git`

---

### Task 1: Create the /fix-pr-review command

**Files:**
- Create: `.claude/commands/fix-pr-review.md`
...
Create `.claude/commands/fix-pr-review.md` with this exact content:
...
```bash
ls -la .claude/commands/fix-pr-review.md
```

Expected: file exists with non-zero size. If missing, re-check the path — it must be `.claude/commands/fix-pr-review.md` relative to the workspace root (`/Users/mykhailoandreitsiv/projects/interactive-tasks`).

- [ ] **Step 4: Commit the command file**

```bash
git add .claude/commands/fix-pr-review.md
git commit -m "feat: add /fix-pr-review slash command"
```

> **Manual verification after commit:** In Claude Code, type `/fix-pr-review https://github.com/andreitsivm/interactive-tasks/pull/1` and confirm Claude fetches the PR, identifies the two PR Agent comments, and begins processing the relevant issues.
